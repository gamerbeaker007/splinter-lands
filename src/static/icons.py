
SPL_WEB_URL = 'https://next.splinterlands.com/'
WEB_URL = 'https://d36mxiodymuqjm.cloudfront.net/'


dec_icon_url = WEB_URL + 'website/icon_dec.png'
sps_icon_url = WEB_URL + 'website/land/resources/sps.png'

grain_icon_url = WEB_URL + 'website/land/resources/grain.png'
stone_icon_url = WEB_URL + 'website/land/resources/stone_500.webp'
wood_icon_url = WEB_URL + 'website/land/resources/wood_500.webp'
iron_icon_url = WEB_URL + 'website/land/resources/iron_500.webp'
research_icon_url = WEB_URL + 'website/land/resources/research.png'
tax_icon_url = WEB_URL + 'website/nav/icon_nav_items_active@2x.png'
